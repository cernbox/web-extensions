...
const basename = (s) => (s.includes('/') ? s.slice(s.lastIndexOf('/') + 1) : s)
...
export default defineComponent({
  ...
  computed: {
    ...
    breadcrumbs() {
      ...
      const path = Array.isArray(item) ? item.join('/') : item
      let breadcrumbsFromPathValue =
        this.$route.params?.item?.length > 0 ? breadcrumbsFromPath(this.$route, path) : []
