import { filterResources, ResourcePreview, ResourcesStore, SearchPreview, SearchResult } from '@ownclouders/web-pkg'
...
export default class Preview implements SearchPreview {
  ...
  public search(term: string): Promise<SearchResult> {
    ...
    const resources = filterResources(this.store.resources as Resource[], term, 5)
    const areHiddenFilesShown: boolean = this.store.areHiddenFilesShown
