<template>
  <oc-table
    ...
    :drag-drop="dragDrop"
    :hover="hover"
    :item-dom-selector="(resource: Resource) => extractDomSelector(resource.id)"
    ...
    <template #actions="{ item }">
      <div v-if="!isResourceDisabled(item)" class="resource-table-actions">
        ...
        <context-menu-quick-action
          ...
          :item="item"
          :resource-dom-selector="(resource: Resource) => extractDomSelector(resource.id)"
...
import { OcButton, OcSpinner, OcTable } from '@ownclouders/design-system/components'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const hasTags = computed(() => width.value >= TAGS_MINIMUM_SCREEN_WIDTH)
